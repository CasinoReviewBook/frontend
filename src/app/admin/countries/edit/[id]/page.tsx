...
export default function EditCountryPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [regionsRes, countryRes] = await Promise.all([
          fetch('http://localhost:4000/api/admin/regions'),
          fetch(`http://localhost:4000/api/admin/countries/${resolvedParams.id}`)
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/admin/countries/${resolvedParams.id}`, {
        method: 'PUT',
